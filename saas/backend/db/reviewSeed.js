...
module.exports = async function reviewSeed(config, env) {
  const { password } = require('../config/demoCredentials').requireDemoCredentials(env);
  ...
  try {
    await db.query(`INSERT INTO usuarios(id,nombre,email,password_hash,rol)
      VALUES($1,'QA Revision SQL','qa-review@example.test',$2,'admin') ON CONFLICT(id) DO NOTHING`, [uid, await bcrypt.hash(password, 8)]);
  ...
  const { token } = await call('/api/auth/login', { email: 'qa-review@example.test', password });
