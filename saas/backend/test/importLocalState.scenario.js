...
module.exports = async ({ db, connect, check }) => {
  ...
  const options = { sourceOwner: owner, targetOwner, targetDatabase: 'contapanama_qa', apply: true,
    expectedHash: preview.fingerprint, confirmation: 'IMPORTAR DOCUMENTOS SIN PUBLICAR' };
  ...
  await assert.rejects(importState(changed,{...options,expectedHash:next.fingerprint},db,{NODE_ENV:'test'}),/no esta vacio/);
