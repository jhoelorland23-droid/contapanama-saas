...
Complete las variables obligatorias de Compose usando `saas/.env.example` como referencia.
Configure `DATABASE_URL` del backend con sus propias credenciales. No hay password predeterminada.

Generar JWT_SECRET seguro (obligatorio antes de iniciar cualquier API):
```bash
