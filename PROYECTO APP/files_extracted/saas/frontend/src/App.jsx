    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#0f1923 0%,#0e3a5c 100%)",display:"flex",alignItems:"center",justifyContent:"center",padding:24}}>
      <div style={{background:C.surface,borderRadius:20,padding:"44px 48px",width:"100%",maxWidth:440,boxShadow:"0 24px 80px rgba(0,0,0,0.4)"}}>
        ...
        <div style={{marginTop:24,padding:"12px 16px",background:C.infoBg,borderRadius:8,fontSize:12,color:C.infoText}}>
          <strong>Demo:</strong> admin@contapanama.pa / [credencial configurada por el operador]
        </div>
      </div>
    </div>
  );
...
// ═══════════════════════════════════════════════════════════════════════════
/**
 * TransaccionesView — Diario Contable (Producción)
 *
...
const TransaccionesView = () => {
  ...
  const BANCOS = ['Banco General', 'Banco Nacional', 'Banistmo', 'BAC', 'Banesco', 'Global Bank', 'Caja de Ahorros', 'MultiBank', 'Scotiabank'];
        ...
        >
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:16 }}>
            <label style={{ display:'flex', alignItems:'center', gap:8, cursor:'pointer' }}>
              <input type='checkbox' checked={form.deducible}
            ...
            {form.deducible && (
              <label style={{ display:'flex', alignItems:'center', gap:8, cursor:'pointer', paddingLeft:24 }}>
                <input type='checkbox' checked={form.tiene_factura}
            ...
            <label style={{ display:'flex', alignItems:'center', gap:8, cursor:'pointer' }}>
              <input type='checkbox' checked={form.itbms_exento}
...
// ═══════════════════════════════════════════════════════════════════════════
/**
 * CalidadView — Control de Calidad Contable (Producción)
 *
      ...
      {!busy && calidad && (
        <>
          <Card style={{ marginBottom:20, display:'flex', alignItems:'center', gap:24, padding:'16px 24px' }}>
            ...
            <button onClick={() => api.pdf(`/api/reportes/calidad-pdf?periodo=${periodo}`).catch(e => alert(e.message))}
        ...
        <div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:16,marginBottom:20}}>
            {[
              ...
              {label:'Calidad PDF',       icon:'shield',   action:()=>api.pdf(`/api/reportes/calidad-pdf?periodo=${periodo}`).catch(e=>alert(e.message)), color:C.danger},
            ].map(r=>(
