...
router.get('/recibos', [
  ...
  try {
    const uid = req.user.id;
    const conds = ['r.usuario_id = $1']; const params = [uid]; let i = 2;
    if (req.query.estado)     { conds.push(`r.estado = $${i++}`);     params.push(req.query.estado); }
    if (req.query.cliente_id) { conds.push(`r.cliente_id = $${i++}`); params.push(req.query.cliente_id); }
