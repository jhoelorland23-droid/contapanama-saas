...
// authMiddleware aplicado por ruta (no como router.use) para no interceptar
// las rutas públicas /auth/aceptar y /auth/login.

cpaOnly.post('/accesos', authMiddleware, [
...
cpaOnly.get('/accesos', authMiddleware, async (req, res) => {
  try {
    const { rows } = await query(`
...
cpaOnly.delete('/accesos/:id', authMiddleware, [param('id').isUUID()], validate, async (req, res) => {
  try {
...
// ── CPA lee/escribe mensajes con un cliente ──────────────────────────────
cpaOnly.get('/cpa/mensajes', authMiddleware, async (req, res) => {
  try {
    const cliente_id = req.query.cliente_id;
    if (!cliente_id) return res.status(422).json({ error: 'cliente_id requerido' });
    const { rows } = await query(`
    ...
    `, [req.user.id, cliente_id]);
    // Marcar mensajes del cliente como leídos por el CPA
    await query(`
      UPDATE portal_mensajes SET leido=true
      WHERE usuario_id=$1 AND cliente_id=$2 AND remitente='cliente' AND leido=false
    `, [req.user.id, cliente_id]);
    res.json({ data: rows });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

cpaOnly.post('/cpa/mensajes', authMiddleware, [
  body('cliente_id').isUUID(),
  ...
  try {
    const { rows } = await query(`
      ...
      VALUES ($1, $2, 'cpa', $3) RETURNING *
    `, [req.user.id, req.body.cliente_id, req.body.contenido]);
    res.status(201).json(rows[0]);
  } catch (err) { res.status(500).json({ error: err.message }); }
});

// Cuántos mensajes no-leídos tiene el CPA por cliente (para el badge)
cpaOnly.get('/cpa/mensajes-sin-leer', authMiddleware, async (req, res) => {
  try {
    const { rows } = await query(`
      SELECT cliente_id, COUNT(*)::int AS n
      FROM portal_mensajes
      WHERE usuario_id=$1 AND remitente='cliente' AND leido=false
      GROUP BY cliente_id
    `, [req.user.id]);
    const out = {};
    rows.forEach(r => { out[r.cliente_id] = r.n; });
    res.json({ porCliente: out, total: rows.reduce((a, r) => a + r.n, 0) });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
// ── GET /api/portal/facturas — facturas emitidas a este cliente ──────────
router.get('/facturas', async (req, res) => {
  try {
    const { cliente_id, usuario_id } = req.portal;
    const { rows } = await query(`
      SELECT id, numero, fecha_emision, subtotal, itbms, total, estado, cufe
      FROM fe_facturas
      WHERE usuario_id=$1 AND cliente_id=$2 AND estado IN ('autorizada','anulada')
...
// ── POST /api/portal/recibos/upload — subida MULTIPART real desde el portal ──
// El archivo entra a la cola OCR del CPA y se procesa igual que si lo subiera él.
const multer = require('multer');
const path = require('path');
const fs = require('fs');
const ocrService = require('../services/ocr');

const portalUploadsDir = path.join(__dirname, '..', 'uploads', 'ocr');
fs.mkdirSync(portalUploadsDir, { recursive: true });
const portalUpload = multer({
  storage: multer.diskStorage({
    destination: portalUploadsDir,
    filename: (req, file, cb) => {
      const ext = path.extname(file.originalname) || '.bin';
      cb(null, `portal-${req.portal.cliente_id}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}${ext}`);
    },
  }),
...
router.post('/recibos/upload', portalUpload.single('archivo'), async (req, res) => {
  try {
    ...
    const { rows } = await query(`
      ...
      RETURNING id
    `, [usuario_id, cliente_id, url, req.file.originalname, req.file.mimetype, req.file.size]);
    const reciboId = rows[0].id;

    // Procesar OCR async (mismo flujo que la subida del CPA)
    ocrService.procesar(req.file.path, req.file.mimetype)
      .then(async (resultado) => {
        const sugerencia = await ocrService.sugerirCuenta(usuario_id, resultado);
        await query(`
      ...
      .catch(async (err) => {
        await query(`UPDATE ocr_recibos SET estado='error', estado_msg=$1 WHERE id=$2`,
          [err.message.slice(0, 500), reciboId]);
      });

    res.status(201).json({ id: reciboId, estado: 'procesando' });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
