...
router.get('/calidad-pdf', [
  ...
  try {
    ...
    // Obtener datos de calidad directamente (sin self-fetch que rompe en producción)
    const { query: dbQuery } = require('../db');
    const CATALOGO = require('../services/catalogo');

    const [{ rows: [cal] }, { rows: descuadres }, { rows: duplicados }, { rows: sinFactura }, { rows: sinCuentaRows }, { rows: conCuenta }, { rows: ingresosSinDebito }, { rows: exentosConItbms }] = await Promise.all([
      dbQuery(`SELECT * FROM v_calidad_periodo WHERE usuario_id=$1 AND periodo=$2`, [uid, periodo]).catch(() => ({ rows: [{}] })),
      dbQuery(`SELECT a.transaccion_id, MIN(t.descripcion) AS descripcion, ROUND(SUM(a.debe)::NUMERIC,2) AS total_debe, ROUND(SUM(a.haber)::NUMERIC,2) AS total_haber, ROUND(ABS(SUM(a.debe)-SUM(a.haber))::NUMERIC,2) AS diferencia FROM asientos_contables a JOIN transacciones t ON t.id=a.transaccion_id WHERE a.usuario_id=$1 AND a.periodo=$2 GROUP BY a.transaccion_id HAVING ABS(SUM(a.debe)-SUM(a.haber)) >= 0.02`, [uid, periodo]),
      dbQuery(`SELECT fecha::text, monto::float, banco, COUNT(*) AS veces FROM transacciones WHERE usuario_id=$1 AND periodo=$2 AND banco IS NOT NULL AND banco<>'' GROUP BY fecha, monto, banco HAVING COUNT(*)>1`, [uid, periodo]),
      dbQuery(`SELECT id, fecha::text, descripcion, monto::float, itbms::float FROM transacciones WHERE usuario_id=$1 AND periodo=$2 AND itbms>0 AND itbms_aplica=true AND tipo='gasto' AND deducible=true AND tiene_factura=false AND COALESCE(itbms_exento,false)=false LIMIT 10`, [uid, periodo]),
      dbQuery(`SELECT id, fecha::text, descripcion, tipo, monto::float FROM transacciones WHERE usuario_id=$1 AND periodo=$2 AND cuenta_contable IS NULL ORDER BY fecha LIMIT 10`, [uid, periodo]),
      dbQuery(`SELECT cuenta_contable FROM transacciones WHERE usuario_id=$1 AND periodo=$2 AND cuenta_contable IS NOT NULL LIMIT 300`, [uid, periodo]),
      dbQuery(`SELECT id, fecha::text, descripcion, monto::float, itbms::float FROM transacciones WHERE usuario_id=$1 AND periodo=$2 AND tipo='ingreso' AND itbms>0 AND (itbms_aplica=false OR itbms_aplica IS NULL) AND COALESCE(itbms_exento,false)=false LIMIT 10`, [uid, periodo]),
      dbQuery(`SELECT id, fecha::text, descripcion, monto::float, itbms::float, tipo FROM transacciones WHERE usuario_id=$1 AND periodo=$2 AND COALESCE(itbms_exento,false)=true AND itbms>0 LIMIT 10`, [uid, periodo]),
    ]);

    const metricas = cal || {};
    const cuentaVacia = conCuenta.filter(t => t.cuenta_contable && !CATALOGO[t.cuenta_contable]).slice(0, 10);
    const total = parseInt(metricas.total_transacciones) || 1;
    const pen = (parseInt(metricas.sin_clasificar)||0)*3 + (parseInt(metricas.sin_asiento)||0)*2 + (parseInt(metricas.itbms_sin_factura)||0)*2 + descuadres.length*10 + duplicados.length*5 + cuentaVacia.length*4 + ingresosSinDebito.length*3 + exentosConItbms.length*2;
    const score = Math.max(0, Math.min(100, 100 - Math.round((pen/total)*100)));
    const calidad = {
      score,
      nivel: score>=90?'excelente':score>=70?'bueno':score>=50?'regular':'requiere_atencion',
      metricas: { total_transacciones: parseInt(metricas.total_transacciones)||0, sin_clasificar: parseInt(metricas.sin_clasificar)||0, sin_asiento: parseInt(metricas.sin_asiento)||0, itbms_sin_factura: parseInt(metricas.itbms_sin_factura)||0, descuadres: descuadres.length, duplicados_potenciales: duplicados.length },
      alertas: { sin_factura: sinFactura, descuadres, sin_cuenta: sinCuentaRows, ingreso_sin_debito: ingresosSinDebito, exento_con_itbms: exentosConItbms },
    };
